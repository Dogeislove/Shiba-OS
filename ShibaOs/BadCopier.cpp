...
namespace tools
{
    ...
    void recovery()
    {
        ofstream ost{ "boot.cfs" };
        ost << "SHIBA32-LOCATION = shib32";
        ost.clear();
    }
}
...
for (; ; ) //Lazy fuckin' loop
 ...
 bool checkva(string name)
 {
     ifstream ist{ "apps/" + name };
     if (!ist) 
     {
         cout << "Error file " << name << " Does not exist. Its either removed, the name was changed or its corrupted." << endl;
         return false;
     }
     if (ist)
     {
...
void choose(string& cho, string& cho1, string& ree)
{
    ...
    if (cho1 == "NF" && checkva("NF.exe" ) == true)
    {
        if (ree == "false") {
    ...
    if (cho1 == "read" && checkva("read.exe") == true)
    {
    ...
    if (cho1 == "write" && checkva("write.exe") == true)
    {
        if (ree == "false") {
    ...
    if (cho1 == "help" && checkva("help.ihl") == true)
    {
        help();
        say = true;
    }
    ...
    //Tools here
    if (cho1 == "rand_math" && checkva("rand_math.exe") == true)
    {
        say = true;
        tools::randmath();
    }
    if (cho1 == "flip" && checkva("flip.exe") == true)
    {
        say = true;
        tools::flip();
    }
...
string readn(string find)
{
    bool sec = false;
    ifstream ist{ "boot.cfs" };
    if (!ist) tools::recovery();
    for (string i; ist >> i;)
    {
        if (sec == false && i == find)
        {
...
bool launch = true;
bool debug = true;
string shib32 = readn("SHIBA32-LOCATION");
